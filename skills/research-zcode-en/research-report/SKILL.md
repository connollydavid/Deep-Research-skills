...
description: Summarize deep research results into a markdown report, cover all fields, skip uncertain values (ZCode/Z.ai edition).
---

# Research Report - Summary Report (ZCode / Z.ai edition)
...
Generate `generate_report.py` in `{topic}/` directory. The script takes the
paths as explicit CLI arguments (no YAML parsing dependency; field structure
is passed via a `--fields-json` argument the skill writes from fields.yaml
as text):
...
- Read field structure from the fields-json argument
...
Run `python {topic}/generate_report.py --results-dir {output_dir} --fields-json {topic}/fields_as_json.json --out {topic}/report.md`
